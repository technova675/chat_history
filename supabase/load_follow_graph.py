"""Load an Apify premium-x-follower-scraper export into followers/following.
...
The two directions go into two tables, unchanged: a mutual is exported twice,
once per direction, and lands as one row in each. Nothing is collapsed here -
network_cards derives "mutual" from membership in both.
...
downgrade a DM counterparty row. An account in both has a row in each.
  ...
  python supabase/load_follow_graph.py --dataset path/to/dataset.json \
      [--owner <owner_id>] [--replace] [--dry-run]
"""
...
# type value in the export -> table it belongs in.
TABLES = {"follower": "followers", "following": "following"}
...
def to_row(u, owner_id, now):
    """Legacy user object -> a followers/following row. Same shape for both."""
    return {
        "owner_id":           owner_id,
        ...
        # network_cards surfaces them as null so the card draws no false badge.
...
def wipe(base, key, table, owner_ids):
    """Drop the owners' existing rows, so an account they no longer follow
    disappears instead of lingering. Scoped to the owners in this export -
    another owner's graph is not ours to delete."""
    for owner_id in sorted(owner_ids):
        q = urllib.parse.quote(owner_id, safe="")
        request(f"{base}/rest/v1/{table}?owner_id=eq.{q}", key, "DELETE",
                prefer="return=minimal")
        print(f"  {table}: cleared owner {owner_id}")
...
def main():
    ...
    ap.add_argument("--replace", action="store_true",
                    help="delete these owners' existing rows in both tables first")
    ...
    # table -> {(owner_id, rest_id): row}. Keyed, because the export can list
    # the same account twice in one direction; last copy wins, being fresher.
    seen    = {t: {} for t in TABLES.values()}
    owners  = {}   # target_username -> owner_id, resolved once each
    skipped = 0

    for u in items:
        rid   = str(u.get("id_str") or u.get("id") or "")
        table = TABLES.get(u.get("type"))
        target = u.get("target_username")
        if not rid or not table or not (args.owner or target):
            skipped += 1
            continue
        ...
        seen[table][(owner_id, rid)] = to_row(u, owner_id, now)

    owner_ids = {r["owner_id"] for t in seen.values() for r in t.values()}
    print(f"{len(items)} items -> "
          + ", ".join(f"{len(rows)} {t}" for t, rows in seen.items())
    ...
    if args.replace:
        for table in TABLES.values():
            wipe(base, key, table, owner_ids)

    print("uploading")
    for table, rows in seen.items():
        upsert(base, key, table, list(rows.values()), "owner_id,rest_id")
    print("done")
