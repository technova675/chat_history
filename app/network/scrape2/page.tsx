...
/**
 * Driver for the SECOND owner's sweep, on the second Apify account. Deliberately
 * a copy of /network/scrape rather than a shared component: the two run side by
 * side in two tabs all evening, and a change made for one of them should not be
 * able to break the other mid-run. The violet accent is how you tell the tabs
 * apart at a glance.
 */
...
export default function NetworkScrapeTwoPage() {
  ...
  const refresh = useCallback(async () => {
    const res = await fetch("/api/network/scrape2");
  ...
  /** One handle: one billable actor run. */
  const runOne = useCallback(
    async (handle: Candidate, index: number, total: number) => {
      ...
      // The other tab got here first. No actor run, nothing spent.
      if (data.alreadyScraped) {
        say(`${prefix} — already scraped by the other sweep, skipped`, "info");
        return;
      }
  ...
  const start = useCallback(
    async (all: boolean) => {
      ...
      try {
        ...
        // concurrent actor runs. Parallel handles would start erroring - the
        // parallelism here is the OTHER tab, on the other token.
    ...
    <main className="min-h-screen bg-[#0b0f14] px-6 py-10 font-mono text-[13px] text-slate-300">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-5">
        <div className="flex flex-wrap items-baseline gap-3">
          <h1 className="text-[15px] text-slate-100">
            network post scrape <span className="text-violet-400">· owner 2</span>
          </h1>
          <a
            href="/network/scrape"
        ...
        <p className="leading-relaxed text-slate-400">
          ...
          Apify account (<span className="text-slate-200">APIFY_TOKEN_2</span>),
          so it is genuinely parallel with the other tab. Billed per result — up
          to about ${cost} for the {queue?.remaining ?? 0} handle(s) left.
        </p>

        <div className="rounded border border-violet-900/60 bg-violet-950/20 px-5 py-4 leading-relaxed text-violet-300">
          Keep this tab open — the loop runs in the browser and stops if the page
          is closed. Safe to run beside the other sweep: both write to{" "}
          <span className="text-violet-200">user_posts</span> keyed on tweet_id,
          and a handle the other tab has already bought is skipped here without
          an actor run.
        </div>

        <div className="relative h-7 w-full overflow-hidden rounded-full border border-slate-700/70 bg-[#111823]">
          <div
            className="absolute inset-y-0 left-0 bg-violet-800/70 transition-[width] duration-500"
            style={{ width: `${pct}%` }}
            role="progressbar"
            aria-valuenow={pct}
            aria-valuemin={0}
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className="rounded border border-violet-700/70 bg-violet-900/30 px-4 py-2 text-violet-300 transition-colors hover:bg-violet-900/60 disabled:cursor-not-allowed disabled:opacity-40"
          >
